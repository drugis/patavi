(ns patavi.worker.pirate.util_test
  (:use clojure.test)
  (:require [patavi.worker.pirate.util :as R]))
...
(deftest test-r-assignments
  (testing "Assignments to RServe"
    ...
    (is (= (R/retrieve @R "foo") {"foo" "foobar"}))
  ...
  (testing "Complexer assignment"
    ...
    (is (= (R/retrieve @R "foo") {"foo" [1 2 3 4]})))
  (testing "Asignment of list of maps"
    (R/assign @R "foo" [{"foo" 10} {"bar" 5}])
    (is (= (R/retrieve @R "foo") [{"foo" 10} {"bar" 5}])))
